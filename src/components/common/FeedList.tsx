...
interface Props {
  ...
  ratings: { value: PostDto['Rating']; count: PostDto['CommentsNum'] };
  selfRating: PostDto['SelfRating'];
  comments: PostDto['Comments'];
}

const FeedList = ({ records, ratings, selfRating }: Props) => {
    ...
    <>
      <div>
        ...
        <Flex justify='space-between' align='center' className='text-2xl mt-2 px-2'>
          {ratings.count > 0 && (
            <Flex gap={4} align='center' className='cursor-pointer' onClick={() => setComments([])}>
              <StarFilled />
